...
def check_action(disruption, config):
    ...
    if not pd.read_sql(search1, conn).values.any():  # If the disruption doesn't exist in the database
        ...
        action = 'new'
    elif not pd.read_sql(search2, conn).values.any():  # If the disruption exists but the situation is different
